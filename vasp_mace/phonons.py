...
def run_phonons(atoms: Atoms, calc, cfg) -> None:
    ...
    if cfg.IBRION == 6:
        try:
            ...
            print(
                "[warn] phonopy not found; IBRION=6 symmetry reduction unavailable. "
...
def _run_brute_force(atoms: Atoms, calc, cfg) -> None:
    ...
    print(
        f"[info] {N} atoms × 3 directions × {nfree} = {n_disp_all} single-point calculations"
    )
    if N > 150:
        print(
            f"[warn] {N} atoms → {n_disp_all} force evaluations. "
    ...
    for i in range(N):
        for alpha in range(3):
            for sign in [+1, -1] if nfree == 2 else [+1]:
                ...
                print(
                    f"  [{count:4d}/{n_disp_all}] atom {i:4d}  {sign_str}{'xyz'[alpha]}",
                    flush=True,
                )
    ...
    n_dynmat = N * 3
    dynmat_entries = _make_dynmat_entries_brute(
        N, forces_blocks, forces_eq, delta, nfree
    )
    ...
    print(
        f"[done] {n_disp_all} displaced configurations complete. "
...
def _run_with_symmetry(atoms: Atoms, calc, cfg) -> None:
    ...
    print(
        f"[info] {n_disp} irreducible displacements "
    ...
    for k, sc in enumerate(supercells):
        ...
        print(
            f"  [{k+1:4d}/{n_disp}] atom {atom_idx:4d}  {sign_str}{'xyz'[alpha]}",
            flush=True,
        )
        f = ase_sc.get_forces()
    ...
    # --- Write output files ---
    _write_dynmat("DYNMAT", atoms, dynmat_entries, n_types, n_dynmat)
    _write_xdatcar_phonons_from_sc("XDATCAR", atoms, disp_structs)
    _write_oszicar_phonons("OSZICAR", energy_eq, [(e,) for e in energies])
    _write_outcar_phonons("OUTCAR", atoms, freqs, eigvecs, cfg)
    write_contcar("CONTCAR", atoms)

    print(
        f"[done] {n_disp} displaced configurations complete. "
        f"Wrote DYNMAT, XDATCAR, OSZICAR, OUTCAR, CONTCAR, "
...
def _check_residual_forces(forces_eq: np.ndarray) -> None:
    ...
    if fmax > 0.05:
        print(
            f"[warn] Max equilibrium force = {fmax:.4f} eV/Å. "
            "Consider relaxing the structure first (IBRION=1/2/3)."
        )


def _displaced_calc(atoms: Atoms, calc, atom_idx: int, disp_vec: np.ndarray):
...
def _force_constants_brute(
    N: int, forces_blocks: list, delta: float, nfree: int, forces_eq: np.ndarray
) -> np.ndarray:
    """
...
def _make_dynmat_entries_brute(
    N: int, forces_blocks: list, forces_eq: np.ndarray, delta: float, nfree: int
) -> list:
    """
...
def _make_dynmat_entries_phonopy(
    atoms: Atoms, ph, disp_infos: list, delta: float
) -> list:
    """
    ...
    for d in disp_infos:
        ...
        for j in range(N):
            for beta in range(3):
                ...
                dynmat_forces[j, beta] = (
                    -ph.force_constants[j, atom_idx, beta, alpha] * delta
                )
...
def _diagonalize(atoms: Atoms, C: np.ndarray):
    ...
    for i in range(N):
        for alpha in range(3):
            for j in range(N):
                for beta in range(3):
                    D[i * 3 + alpha, j * 3 + beta] = C[i, alpha, j, beta] / math.sqrt(
                        masses[i] * masses[j]
                    )
...
def _write_dynmat(
    path: str, atoms: Atoms, entries: list, n_types: int, n_displacements: int
) -> None:
    """
    ...
    with open(path, "w") as f:
        ...
        for atom_idx, alpha, disp_vec, dynmat_forces in entries:
            ...
            f.write(
                f"  {atom_idx + 1:3d}  {direction:3d}"
                f"  {disp_vec[0]:6.4f}  {disp_vec[1]:6.4f}  {disp_vec[2]:6.4f}\n"
            )
            for j in range(N):
                f.write(
                    f"{dynmat_forces[j, 0]:16.12f} "
                    f"{dynmat_forces[j, 1]:16.12f} "
                    f"{dynmat_forces[j, 2]:16.12f}\n"
                )


def _write_xdatcar_phonons(path: str, atoms: Atoms, blocks_all: list) -> None:
...
def _write_xdatcar_phonons_from_sc(path: str, atoms: Atoms, disp_structs: list) -> None:
...
def _write_oszicar_phonons(path: str, energy_eq: float, energies: list) -> None:
...
def _write_outcar_phonons(
    path: str, atoms: Atoms, freqs: np.ndarray, eigvecs: np.ndarray, cfg
) -> None:
    ...
    with open(path, "w") as f:
        f.write(
            " vasp-mace  phonon calculation (IBRION={cfg.IBRION})\n".format(cfg=cfg)
        )
        ...
        for m in range(n_modes):
            ...
            for i, (pos, d) in enumerate(zip(positions, ev)):
                f.write(
                    f"    {pos[0]:10.6f}{pos[1]:10.6f}{pos[2]:10.6f} "
