...
- `vasp_mace.heat.heat_flux.HeatFluxCalculator` abstract interface and `make_heat_flux_calculator` factory; `MACEUnfoldedHeatFluxCalculator` adapter wrapping [`mace-unfolded`](https://github.com/pulgon-project/mace-unfolded) for the autograd-based unfolded-cell potential heat flux. `mace_unfolded` ships as an opt-in extra, installed with `pip install vasp-mace[heat]`. Default install (without `[heat]`) is unchanged. The MD loop is not yet wired into this backend — that wiring lands in the next stage.
